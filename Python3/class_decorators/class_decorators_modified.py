...
def invariant(predicate):
    ...
    Returns:
        A class decoartor for checking the class invariant tested by the
    ...
    def invaruant_checking_class_decorator(cls):
        ...
        property_names = [name for name, attr in vars(
            cls).items() if isinstance(attr, property)]
...
if __name__ == "__main__":
    ...
    p = Temperature(5.0)
    # p2 = Temperature(-5.0)  # will raise the error, failed invariant check
    s = Temperature(42.0)  # set kelvin temperature
    # s.set_kelvin(-1.0)     # modify kelvin temperature, RuntimeError is raised
    t = Temperature(42.0)
    print(t.celsius)
    t.celsius = -100
    print(t.celsius)
    pp(vars(Temperature))  # get __dict__ attributes of Temperature class
    t.fahrenheit = 100
    print(t.fahrenheit)
    # t.celsius = -300  # no exception is raised, class invariant violated
    # t.set_kelvin(1e33)  # set kelvin above absolute hot, RuntimeError raised
