...
class TetstContainerProtocol(unittest.TestCase):
...
class TestSizedProtocol(unittest.TestCase):
    ...
    def test_with_dubplicates(self):
        s = SortedSet([5, 5, 5])
        self.assertEqual(len(s), 1)
...
class TetstSquenceProtocol(unittest.TestCase):
    ...
    def test_index_one_beyound_the_beginning(self):
        with self.assertRaises(IndexError):
            self.s[-6]
...
class TestSetOperationsMethods(unittest.TestCase):
    ...
    def test_difference(self):
        s = SortedSet({1, 2, 3})
        t = [2, 3, 4]
        self.assertEqual(s.diffence(t), SortedSet({1}))
